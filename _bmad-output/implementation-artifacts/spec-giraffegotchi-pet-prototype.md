...
**Approach:** A PlatformIO + Arduino firmware for the ESP32-2432S028R. Pure pet-state logic (hunger stat, mood) is kept hardware-free so it can be unit-tested natively; `main.cpp` drives timing, reads the XPT2046 touch panel for a "feed" hit-zone, draws the hunger bar and feed button with TFT_eSPI primitives, and renders the giraffe from full-color PNG assets on LittleFS (decoded with PNGdec). [Renegotiated 2026-06-25 — see Spec Change Log.]
...
**Never:**
- Photographic/3D giraffe art — keep it flat illustrated PNGs regenerable via `tools/gen_giraffe.py`.
...
- **2026-06-25 — Giraffe art moved from TFT primitives to PNG-on-LittleFS.** Human renegotiated the frozen "Never: bitmap assets" boundary to get real character art. Added `board_build.filesystem = littlefs` + `bitbank2/PNGdec`; `data/giraffe_{happy,hungry}.png` generated by `tools/gen_giraffe.py` (PIL, 4x supersample); `drawGiraffe()` now decodes the PNG via PNGdec + `pushImage` (requires `setSwapBytes(true)`). Hunger bar + feed button stay primitives. Flash order: `pio run -t uploadfs` then `-t upload`. KEEP: pet logic + tests untouched (still 7/7); image bg baked to 0xAEDC so it tiles seamlessly over the screen fill.
